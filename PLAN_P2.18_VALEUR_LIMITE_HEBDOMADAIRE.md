...
implicite de la limite hebdomadaire à partir de toutes les sources locales :

```text
valeur implicite = coût API équivalent de tous les événements locaux observé / fraction de quota hebdomadaire consommée
```

Le calcul courant porte sur une fenêtre glissante d’environ deux heures. La
table des resets doit également afficher le coût de toutes les sources locales
observé pendant chaque cycle hebdomadaire terminé et, lorsque le cycle s’est
terminé avec du quota
...
- Réutiliser l’archive SQLite v4 et calculer les métriques à la lecture dans
  ...
  nécessaire.
- Valoriser tous les événements `token_usage_events` des sources locales
  connues (Codex, OpenCode et Hermes), sans dépendre des filtres d’affichage.
- Ne pas faire dépendre l’estimation des filtres d’affichage source/modèle :
  elle représente le quota hebdomadaire suivi localement, tandis que les
  graphiques de tokens existants conservent leur comportement filtrable.
...
1. rechercher le relevé valide le plus proche de `t1 - 7200 s` ;
2. accepter une durée réelle comprise entre 1 h 45 et 2 h 15 afin de tolérer
...
6. sommer sur exactement `[t0, t1)` le coût API équivalent de tous les
   événements locaux, au niveau fournisseur/modèle avant agrégation ;
...
- Entre 0,5 et 1 point, ou en présence d’événements de qualité
  ...
  `low_confidence`.
- À partir de 1 point, avec une fenêtre proche de deux heures, des prix complets et
...
3. sommer les événements locaux tarifables du début du cycle au reset courant ;
...
- Isoler des helpers purs pour la validation des nombres, la tarification d’un
  intervalle toutes sources, la résolution des fenêtres et la qualification des
  points.
...
- Ajouter une carte large dédiée avec titre, description « fenêtre glissante
  de 2 h », devise, légende de qualité, graphique de tendance et état vide.
...
- Vérifier la conversion pourcentage/fraction, l’alignement exact de la fenêtre
  et l’inclusion des événements Codex, OpenCode et Hermes.
...
- Documenter dans `README.md` la formule, le périmètre de toutes les sources
  locales, la fenêtre, les seuils, le lissage, les statuts de qualité, les
  causes de `N/A` et les deux
  colonnes de cycle.
