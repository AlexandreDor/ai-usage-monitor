...
assert_eq false "$(python3 -c 'import json,sys; print(str("breakdown_pagination" in json.load(open(sys.argv[1]))["tokens"]).lower())' "${TEST_ROOT}/analytics-error")" "legacy analytics response exposed pagination metadata"
...
assert_eq 50 "$(json_field "${TEST_ROOT}/analytics-pagination" tokens.breakdown_pagination.limit)" "token breakdown page size"
