...
When Gist synchronization is enabled, the optional `codex_forecast` object is included in both `data.json` and rolling `history.json`. Display thresholds stay only in the current snapshot, while probability observations and generation times are retained in history and in the independent SQLite `forecast_samples` table. Quota anomalies are retained in the SQLite `quota_anomalies` table and delivered through the durable alert journal. Journaled anomalies and inactive detector state follow `ARCHIVE_RETENTION_DAYS`; pending anomalies are retained until journal registration succeeds. Existing schema v1, v2, and v3 archives migrate transactionally to schema v4 during the next writable monitor or `--check` run.
...
- Long-term history is stored separately in `runtime/usage-history.sqlite3`: all limit and Forecast points are kept for 24 hours, then each series is compacted independently. Analytics uses 15-minute buckets through 48 hours, 30-minute buckets through 30 days, then hourly buckets. Token events, reconstructed resets, and quota anomalies use the same retention period without limit-series downsampling. The default retention is one year; `ARCHIVE_RETENTION_DAYS=0` keeps it indefinitely. SQLite schema v4 migrates recognized v1/v2/v3 archives automatically. SQLite is provided by Python's standard library; the raw archive is not served by `serve.sh` or synchronized to the Gist.
